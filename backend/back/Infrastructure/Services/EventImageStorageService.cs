...
{
    private const string EventImageRoutePrefix = "/api/event-images";
    ...
    private readonly string _eventImageRootPath;

    public EventImageStorageService(IWebHostEnvironment environment)
    {
        _eventImageRootPath = Path.Combine(environment.ContentRootPath, "App_Data", "event-images");
        Directory.CreateDirectory(_eventImageRootPath);
    }

    public async Task<string> SaveEventImageAsync(Guid eventId, IFormFile file, CancellationToken cancellationToken)
    {
        if (file.Length > MaxImageBytes)
        {
            throw new InvalidOperationException("Please choose an image under 20MB.");
        }
        ...
        var fileName = $"{eventId:N}_{DateTime.UtcNow:yyyyMMddHHmmssfff}_{Guid.NewGuid():N}{extension}";
        var filePath = Path.Combine(_eventImageRootPath, fileName);
        ...
        return $"{EventImageRoutePrefix}/{Uri.EscapeDataString(fileName)}";
    }

    public Stream? OpenEventImageReadStream(string fileName, out string contentType)
    {
        ...
        var filePath = Path.Combine(_eventImageRootPath, safeFileName);
    ...
    public void DeleteManagedEventImage(string? imageUrl)
    {
        ...
        if (!pathWithoutQuery.StartsWith($"{EventImageRoutePrefix}/", StringComparison.OrdinalIgnoreCase))
        {
            return;
        }

        var fileName = Path.GetFileName(Uri.UnescapeDataString(pathWithoutQuery[(EventImageRoutePrefix.Length + 1)..]));
